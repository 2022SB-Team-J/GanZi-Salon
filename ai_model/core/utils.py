...
#     N, C, H, W = x_src.size()
#     # wb = torch.ones(1, C, H, W).to(x_src.device)          # wb = [0,0] 흰색 빈칸
#     # x_src_with_wb = torch.cat([wb, x_src], dim=0)         # x_src 사진 삭제 (위쪽라인))
#     #x_src_with_wb = torch.cat([wb], dim=0)                 # x_src_sith_wb -> 말그대로 x_src랑 흰색 빈칸이랑 묶어주는거 -> x_src만 있어도됨
...
#     for i, s_ref in enumerate(s_ref_list):            # s_ref_list : ref사진 리스트
#         x_fake = nets.generator(x_src, s_ref, masks=masks)  # 실질적 합성
#         # x_fake_with_ref = torch.cat([x_ref[i:i+1], x_fake], dim=0)    # x_ref 사진 삭제 (왼쪽라인)
...
#     x_concat = torch.cat(x_concat, dim=0)
#     save_image(x_concat,filename)
#     # save_image(x_concat, N+1, filename) # x_concat으로 하면 ref, src 사진 다 나오게 됨, N+1하면 마지막 검정빈칸 나옴
#     #save_image(x_fake, N+1, filename)  # x_fake으로 하면, 남자만 나옴
#     del x_concat
