...
# @api_router.post("/api/auth/join")
# async def 회원가입(user:User):
#     new_user = JoinUser()
#     new_user.user_id = user.email
...
@api_router.get("/api/auth/join/check")
async def 확인(user:User):
    print(user)
    return user
...
def read_root():
    return {"Hello": "World"}
@api_router.post("/api/auth/join")
async def login_user(join_user: JoinUser):
    print(join_user)
    return join_user
